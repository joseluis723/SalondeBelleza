...
// Desactivar al profesional (deja de aparecer, pero conserva el historial)
...
// ELIMINAR al profesional definitivamente (solo administrador)
router.delete('/:id', requireRole('admin'), asyncHandler(async (req, res) => {
  const existing = await pool.query('SELECT id FROM professionals WHERE id = $1', [req.params.id]);
  if (existing.rowCount === 0) return res.status(404).json({ error: 'Profesional no encontrado.' });

  const appts = await pool.query('SELECT COUNT(*) AS total FROM appointments WHERE professional_id = $1', [req.params.id]);
  const total = Number(appts.rows[0].total);

  if (total > 0) {
    return res.status(409).json({
      error: `No se puede eliminar: tiene ${total} cita(s) registradas. Puedes desactivarlo para que deje de aparecer.`,
      can_deactivate: true,
      appointments: total
    });
  }

  await pool.query('DELETE FROM professional_service_commissions WHERE professional_id = $1', [req.params.id]);
  await pool.query('UPDATE users SET professional_id = NULL WHERE professional_id = $1', [req.params.id]);
